...
window.addEventListener("load",function(){
	// Check if lightbox is enabled via the custom setting
	const postContent = document.querySelector(".post-content");
	if(!postContent || !postContent.classList.contains("enable-lightbox")) return;
	
	const e=document.querySelector(".post-content .kg-card img"),t=document.querySelectorAll(".post-content .kg-image-card a img");
	if(t)for(var n=0;n<t.length;n++)t[n].classList.add("no-lightense");
	e&&Lightense(".post-content .kg-image-card img:not(.no-lightense),.post-content .kg-gallery-card img:not(.no-lightense)",{padding:60,offset:0})
},!1);
...
/* Member CTA Popup Modal
   –––––––––––––––––––––––––––––––––––––––––––––––––––– */
!function(){
	const modalCTA = document.querySelector('.members-cta.is-modal');
	if(!modalCTA) return;
	...
	modalCTA.insertBefore(closeBtn, modalCTA.firstChild);
	...
	document.addEventListener('keydown', function(e) {
		if(e.key === 'Escape' && modalCTA) {
			modalCTA.style.display = 'none';
		}
	});
	
	// Show modal after a delay (3 seconds) or on scroll
	...
	// Or show when user scrolls 50% down the page
	let scrollTriggered = false;
	window.addEventListener('scroll', function() {
		if(scrollTriggered || modalShown) return;
